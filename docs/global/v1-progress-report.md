...
`createInteractionModule(host)` 组合调查与对话，`mountExploration(...)` 把它挂到游戏页面。全局负责提供宿主和容器，探索模块负责具体热点、鼠标点击、调查反馈、对白阅读与确认、背包展示。
...
| 2 | 正式成就入口曾对所有读档失败都使用初始状态 | 已修复：无存档、坏档、版本错误、身份不匹配和存储不可用分别显示玩家提示，不再用初始状态掩盖读取失败 | 已完成 |
...
| 5 | 正式页面曾显示技术节点 ID、任务 ID、“等待对应功能模块返回结果”和事件日志 | 已修复：正式页面删除节点状态栏、事件日志和命令详情；拼图状态改为玩家语言；内部错误保留在控制台，页面只显示可执行提示。三种宽度的视觉验收仍需页面/视觉/探索共同完成 | 信息隔离已完成；视觉验收待共同完成 |
...
环境：Node `v24.13.0`。本次没有重新进行真实浏览器的鼠标和视觉全站验收；用户此前报告的浏览器跑通记录可作为另一份证据，不能与本次代码测试混称。
