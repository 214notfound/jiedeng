...
<!-- 本文件用于约定《借灯》V1 各模块的交接方式、英文命名和联调规则；全组确认后作为 PR 审核依据。 -->
...
| 游戏展示名：`《借灯》` 或其他 | 影响封面、README、介绍页和素材 | 正式展示名使用 `《借灯》` | 项目组（2026-09-05 本轮确认） |
...
| 公共视觉风格和素材来源 | 美术决定风格，前端负责实现 | About 页面采用冷蓝雨夜、暖灯强调与半透明叙事面板；复用项目封面图，成员头像由本人提供，成员左栏背景为基于项目画面的 AI 编辑衍生图，来源说明见 `assets/images/about/README.md` | 项目组（2026-09-05 本轮确认） |
