...
export function bindHost({getContext, subscribe}, validate) {
  ...
  function onChange(listener) {
    ...
    const cancel = subscribe(() => {
      ...
      try { read(); } catch (error) { console.error("[achievement-host] 读取失败。", error); }
      try { listener(); } catch (error) { console.error("[achievement-host] 界面更新失败。", error); }
    });
    ...
    const stop = () => {
      ...
      stopped = true;
      stops.delete(stop);
      try { cancel(); } catch (error) { console.error("[achievement-host] 清理失败。", error); }
    };
    stops.add(stop);
    return stop;
  }
